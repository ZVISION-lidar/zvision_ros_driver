...
public:
  ...
  virtual int getPacket(zvision_lidar_msgs::msg::ZvisionLidarPacket* pkt, const double time_offset) = 0;
...
{
public:
  InputSocket(rclcpp::Node * private_nh, uint16_t port = UDP_DATA_PORT_NUMBER);

  virtual ~InputSocket();

  virtual int getPacket(zvision_lidar_msgs::msg::ZvisionLidarPacket* pkt, const double time_offset);
...
{
public:
  InputPCAP(rclcpp::Node * private_nh, uint16_t port = UDP_DATA_PORT_NUMBER, double packet_rate = 0.0,
