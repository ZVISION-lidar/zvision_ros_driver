...
{
static const size_t packet_size = sizeof(zvision_lidar_msgs::zvisionLidarPacket().data);
...
int InputSocket::getPacket(zvision_lidar_msgs::zvisionLidarPacket* pkt, const double time_offset)
{
  ...
  : Input(private_nh, port), packet_rate_(packet_rate), filename_(filename)
{
  ...
  {
    ROS_FATAL("Error opening zvision lidar socket dump file.");
    return;
  }
...
/** @brief Get one zvision lidar packet. */
int InputPCAP::getPacket(zvision_lidar_msgs::zvisionLidarPacket* pkt, const double time_offset)
{
  ...
  while (flag == 1)
  {
    ...
    {
      ROS_WARN("Error %d reading zvision lidar packet: %s", res, pcap_geterr(pcap_));
      return -1;
    }
    ...
    ROS_DEBUG("replaying zvision lidar dump file");
